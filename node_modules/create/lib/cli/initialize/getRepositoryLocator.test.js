...
describe("getRepositoryLocator", () => {
    it("throws an error when the options are missing owner", () => {
        ...
        }).toThrowError(`To run with --mode initialize, --owner must be a string-like, not undefined.`);
    });
    it("throws an error when the options have an invalid owner", () => {
        const options = { owner: [1, 2, 3], repository };
        expect(() => {
            getRepositoryLocator(options);
        }).toThrowError(`To run with --mode initialize, --owner must be a string-like, not object.`);
    });
    it("throws an error when the options are missing repository", () => {
        const options = { owner };
        expect(() => {
            getRepositoryLocator(options);
        }).toThrowError(`To run with --mode initialize, --repository must be a string-like, not undefined.`);
    });
    it("throws an error when the options have an invalid repository", () => {
        const options = { owner, repository: [1, 2, 3] };
        expect(() => {
            getRepositoryLocator(options);
        }).toThrowError(`To run with --mode initialize, --repository must be a string-like, not object.`);
    });
...
//# sourceMappingURL=getRepositoryLocator.test.js.map